...
class OrderedBehavior extends ModelBehavior {
	 ...
	 */
	public function moveto(&$Model, $id = null,$foreign_key, $new_weight =null) {
            //echo $id.'=='.$foreign_key.'=='.$new_weight;die;
		if (!$id || !$new_weight || $new_weight < 1) {
			return false;
		}
                
		$highest = $this->_highest($Model,$foreign_key);
               // echo $highest[$Model->alias][$this->settings[$Model->alias]['field']];
                //die;
	...
	private function _highest(&$Model,$foreign_key=null) {
		$options = array(
				'order' => $this->settings[$Model->alias]['field'] . ' DESC', 
				'fields' => array($Model->primaryKey, $this->settings[$Model->alias]['field']), 
				'recursive' => -1);
		if ($this->settings[$Model->alias]['foreign_key']) {
			if (empty($Model->data) || !isset($Model->data[$Model->alias][$this->settings[$Model->alias]['foreign_key']])) {
				$this->_read($Model, $Model->id);
                        ...
                        if($foreign_key!=''){
			$options['conditions'] = array( $Model->alias . '.' . $this->settings[$Model->alias]['foreign_key'] => $foreign_key);
                        }else{
					$options['conditions'] = array($Model->alias . '.' . $this->settings[$Model->alias]['foreign_key'] => $Model->data[$Model->alias][$this->settings[$Model->alias]['foreign_key']]);
